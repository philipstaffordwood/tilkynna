...
public class JwtAccessTokenCustomizer extends DefaultAccessTokenConverter implements JwtAccessTokenConverterConfigurer {
    ...
    /**
     * Spring oauth2 expects roles under authorities element in tokenMap, but Keycloak provides it under resource_access. Hence extractAuthentication method is overridden to extract roles from resource_access.
     *
    ...
    public OAuth2Authentication extractAuthentication(Map<String, ?> tokenMap) {
        ...
        OAuth2Request request = new OAuth2Request(oAuth2Request.getRequestParameters(), oAuth2Request.getClientId(), authorities, true, oAuth2Request.getScope(), audienceList, null, null, null);
    ...
    private List<GrantedAuthority> extractRoles(JsonNode jwt) {
        ...
        jwt.path(CLIENT_NAME_ELEMENT_IN_JWT).elements().forEachRemaining(e -> e.path(ROLE_ELEMENT_IN_JWT).elements().forEachRemaining(r -> rolesWithPrefix.add("ROLE_" + r.asText())));
